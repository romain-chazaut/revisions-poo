...
class Product {
    ...
    public function update(PDO $pdo) {
        if ($this->id === 0) {
            return false; // L'ID du produit n'est pas défini
        }

        $query = "UPDATE product SET name = :name, photos = :photos, price = :price, description = :description, quantity = :quantity, updated_at = :updated_at, category_id = :category_id WHERE id = :id";
        $stmt = $pdo->prepare($query);
